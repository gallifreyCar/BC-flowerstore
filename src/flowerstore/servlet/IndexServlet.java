...
import flowerstore.bean.Dingdan;
import flowerstore.bean.Gonggao;
import flowerstore.bean.Gouwuche;
import flowerstore.bean.Product;
import flowerstore.bean.User;
import flowerstore.dao.DingdanDao;
import flowerstore.dao.GonggaoDao;
import flowerstore.dao.GouwucheDao;
import flowerstore.dao.ProductDao;
import flowerstore.dao.UserDao;
import flowerstore.util.QrCoreMake;
import flowerstore.util.Util;


//后台servlet，用于处理前台的所有请求
public class IndexServlet extends HttpServlet {
    public String qrCodeServerAddress;

    private static final long serialVersionUID = 1L;

    public void init(ServletConfig config) throws ServletException {
        super.init(config);
        qrCodeServerAddress = config.getInitParameter("qrCodeServerAddress");
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        //跳转到查看商品详情页面
            ...
            String qrCodePath = getServletContext().getRealPath("myqrcode/" + id + ".png");
            File qrCodeFi = new File(qrCodePath);
            if (!qrCodeFi.exists()) {
                qrCodeFi.getParentFile().mkdirs();
                QrCoreMake.creatQrcode(qrCodeServerAddress + "/indexServlet/productupdate?id=" + id, qrCodePath);
            }
